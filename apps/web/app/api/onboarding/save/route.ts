...
import { createUser, getUserByPhone } from '@nutricoach/core';
import { getServiceClient } from '@nutricoach/core';
import type { Goal, Gender, ActivityLevel, DietaryPreference } from '@nutricoach/core';

// Helper functions inlined here to avoid importing server-only packages client-side
...
export async function POST(req: NextRequest) {
  ...
  const userData = {
    // phone is not collected in this flow — use a placeholder that won't conflict
  ...
  // Use the service client from packages/core/src/db to save to users table
